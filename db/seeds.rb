...
def seed_instructions 
    30.times do |count|
        instruction = Instruction.create(title: Faker::Name.unique.name)
    end 
    seed_repairs()
end
...
def seed_steps 
    Repair.all.each_with_index do |element, index|
        5.times do |count|
            step = element.steps.create(title: "#{element.title}.#{count + 1}", description: Faker::Lorem.paragraph(sentence_count: 2)) 
        end 
    end 
end 
